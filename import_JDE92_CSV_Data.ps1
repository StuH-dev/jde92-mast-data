param(
    [string]$CsvDirectory = "F:\SCHEDULED_JOBS\JDE92_Master_Data\DATA_FILES",
    [string]$ServerInstance = 'AU-SRV-SQL-04',
    [string]$Database = 'MASTER_DATA_V2',
    [string]$Username = 'sa',
    [string]$Password = 'NdiZqp49svQp3h8m',
    [switch]$IntegratedSecurity,
    [bool]$TruncateBeforeImport = $true,
    [string]$TempFolder = "F:\SCHEDULED_JOBS\JDE92_Master_Data\DATA_FILES",
    [string]$LogPath = "F:\SCHEDULED_JOBS\JDE92_Master_Data\DATA_FILES"
)
...
function Import-CsvToSql {
    param(
        ...
        [string]$TempFolder = "F:\SCHEDULED_JOBS\JDE92_Master_Data\DATA_FILES",
        [string]$LogPath = "F:\SCHEDULED_JOBS\JDE92_Master_Data\LOGS"
    )
    ...
    $logFilePath = Join-Path $LogPath "${timestamp}_import_run.log"
    ...
    function Invoke-SqlBulkInsert {
        ...
        try {
            ...
            foreach ($row in $csvData) {
                ...
                foreach ($colName in $MatchingColumns) {
                    $propertyExists = $row.PSObject.Properties.Name -contains $colName
                    if ($propertyExists) {
                        $val = $row.$colName
                    } else {
                        $val = $null
    ...
    function Invoke-SqlMerge {
        ...
        try {
            ...
            foreach ($colName in $MatchingColumns) {
                ...
                $collation = ""
                if ($dataType -like "*char*" -or $dataType -like "*text*" -or $dataType -like "*nchar*" -or $dataType -like "*nvarchar*" -or $dataType -like "*varchar*") {
                    $collation = " COLLATE DATABASE_DEFAULT"
                }
                
                $nullability = if ($isNullable) { "NULL" } else { "NOT NULL" }
                $columns += "[$colName] $sqlType$collation $nullability"
            }
            $createTempTableQuery += ($columns -join ", ") + ")"
            ...
            $buildComparison = {
                ...
                $dataType = $TableColumnInfo[$colName].DataType
                if ($dataType -like "*char*" -or $dataType -like "*text*" -or $dataType -like "*nchar*" -or $dataType -like "*nvarchar*" -or $dataType -like "*varchar*") {
                    "t.[$colName] COLLATE DATABASE_DEFAULT = rd.[$colName] COLLATE DATABASE_DEFAULT"
                } else {
                    "t.[$colName] = rd.[$colName]"
                }
            }
            
            $dedupeJoinConditions = ($primaryKeyColumns | ForEach-Object { & $buildComparison $_ }) -join " AND "
...
INNER JOIN RankedData rd ON $dedupeJoinConditions
WHERE rd.rn > 1
"@
            ...
            $buildComparison = {
                ...
                $dataType = $TableColumnInfo[$colName].DataType
                if ($dataType -like "*char*" -or $dataType -like "*text*" -or $dataType -like "*nchar*" -or $dataType -like "*nvarchar*" -or $dataType -like "*varchar*") {
                    "t.[$colName] COLLATE DATABASE_DEFAULT = s.[$colName] COLLATE DATABASE_DEFAULT"
                } else {
                    "t.[$colName] = s.[$colName]"
                }
            }
            ...
            $pkMatchConditions = ($primaryKeyColumns | ForEach-Object { & $buildComparison $_ }) -join " AND "
        ...
        try {
            ...
            if ($TruncateBeforeImport) {
                ...
                if ($processingConfig.Found) {
                    ...
                    Write-Host "Global TruncateBeforeImport = true (trumping table-level setting). Hash table match: '$fileName' -> key '$($processingConfig.Key)' | Truncate: $shouldTruncate | ProcessType: $processType" -ForegroundColor Yellow
                    Write-Log "File '$fileName' matched hash table key '$($processingConfig.Key)'. Global TruncateBeforeImport trumps table-level setting. Truncate: $shouldTruncate, ProcessType: $processType" "INFO"
                } else {
                    $processType = "BulkInsert"
                    Write-Host "Global TruncateBeforeImport = $shouldTruncate" -ForegroundColor Yellow
                    Write-Log "File '$fileName' not in hash table. Using global TruncateBeforeImport: $shouldTruncate" "INFO"
                }
            } elseif ($processingConfig.Found) {
